...
struct ExplicitEncoding: BoSyEncoding {
    ...
    func tauNextStateAssertion(state: Int, _ inputs: BooleanAssignment, nextState: Int, bound _: Int) -> Logic {
        tau(state, inputs, nextState)
    }
    ...
    func tau(_ fromState: Int, _ inputs: BooleanAssignment, _ toState: Int) -> Proposition {
        Proposition("τ_\(fromState)_\(bitStringFromAssignment(inputs))_\(toState)")
    }
    ...
    mutating func solve(forBound bound: Int) throws -> Bool {
        ...
        if case let .sat(assignments) = result {
