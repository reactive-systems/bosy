...
struct StateSymbolicEncoding: BoSyEncoding {
    ...
    func getEncoding(forBound bound: Int) -> Logic? {
        ...
        let lambdaPropositions: [Proposition] = automaton.states.map { lambdaProposition($0) }
        let lambdaSharpPropositions: [Proposition] = automaton.states.map { lambdaSharpProposition($0) }
