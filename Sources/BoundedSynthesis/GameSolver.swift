...
class UCWGame: SafetyGame {
    ...
    init(manager: CUDDManager, automaton: CoBüchiAutomaton, inputs: [String], outputs: [String], bound k: Int) {
        ...
        controllables = outputs.map { _ in manager.newVar() }
        uncontrollables = inputs.map { _ in manager.newVar() }
        latches = safetyAutomatonStates.map { _ in manager.newVar() }

        compose = controllables + uncontrollables + (0 ..< latches.count).map { _ in manager.zero() }
        initial = latches.reduce(manager.one()) { states, state in states & !state }
        safetyCondition = manager.one()

        let composeOffset = controllables.count + uncontrollables.count

        controllableNames = outputs
        uncontrollableNames = inputs
        latchNames = safetyAutomatonStates.map { $0.description }
        ...
        for (state, encoded) in zip(safetyAutomatonStates, latches) {
            if let localSafetyCondition = safetyAutomaton.safetyConditions[state] {
                safetyCondition &= !(encoded & !localSafetyCondition.accept(visitor: cuddEncoder))
                // print("\(state): \(localSafetyCondition)")
            }

            if let outgoing = safetyAutomaton.transitions[state] {
                for (target, transitionGuard) in outgoing {
                    let index = safetyAutomatonStates.firstIndex(of: target)!
        ...
        for initialState in safetyAutomaton.initialStates {
            if let localSafetyCondition = safetyAutomaton.safetyConditions[initialState] {
                safetyCondition &= !(initial & !localSafetyCondition.accept(visitor: cuddEncoder))
...
public class SafetyGameReduction: BoSyEncoding, SingleParamaterSearch {
    ...
    var safetyGame: UCWGame?
    var solver: SafetyGameSolver?
    var winningRegion: CUDDNode?
    ...
    public func solve(forBound bound: RejectingCounter) throws -> Bool {
        try solve(forBound: bound.value)
    }
    ...
    public func extractSolution() -> TransitionSystem? {
        ...
        guard let game = safetyGame else {
            fatalError()
        }
