...
func modelCheckAiger(specification: SynthesisSpecification, implementation: UnsafeMutablePointer<aiger>) throws -> Bool {
    ...
    return try withTemporaryFile(dir: nil, prefix: "", suffix: ".smv", deleteOnClose: true) {
        (tempFile: TemporaryFile) throws -> Bool in
        try withTemporaryFile(dir: nil, prefix: "", suffix: ".aig", deleteOnClose: true) {
            (monitorFile: TemporaryFile) throws -> Bool in
            try withTemporaryFile(dir: nil, prefix: "", suffix: ".aig", deleteOnClose: true) {
                (implementationFile: TemporaryFile) throws -> Bool in
                try withTemporaryFile(dir: nil, prefix: "", suffix: ".aig", deleteOnClose: true) {
                    (combinedFile: TemporaryFile) throws -> Bool in
                    tempFile.fileHandle.write(Data(smvSecification.utf8))
                    try TSCBasic.Process.checkNonZeroExit(arguments: ["./Tools/smvtoaig", "-L", "Tools/ltl2smv", tempFile.path.pathString, monitorFile.path.pathString])

                    aiger_open_and_write_to_file(implementation, implementationFile.path.pathString)

                    let combiner = TSCBasic.Process(arguments: ["./Tools/combine-aiger", monitorFile.path.pathString, implementationFile.path.pathString])
                    try combiner.launch()
                    let combinationResult = try combiner.waitUntilExit()
                    let sequence: [UInt8] = try combinationResult.output.get()

                    combinedFile.fileHandle.write(Data(sequence))

                    let modelCheckOutput = try TSCBasic.Process.checkNonZeroExit(arguments: ["./Tools/aigbmc", combinedFile.path.pathString])
